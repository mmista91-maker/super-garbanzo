...
def fix_list_of_wagons(each_wagons_id, missing_wagons):
    ...
    """
    first, second, locomotive, *rest = each_wagons_id # unpack first 2     
    return [locomotive, *missing_wagons, *rest, first, second] #join as list in order and *missing to avoid list in list
...
def fix_wagon_depot(wagons_rows):
    ...
    #make list for every row where row is extracted from wagon rows. extraction by * unpacking external list and zip() changing     columns to row
